...
const programName: string = 'marchProg'

type MarchProgramInfo = {
...
type MarchBufferCollection = {
  vertPosition: WebGLBuffer | null
  // circlePosition: WebGLBuffer | null
}

function initBuffers(gl: WebGLRenderingContext): MarchBufferCollection {
...
// function setCirclePositionAttribute(gl: WebGLRenderingContext, programInfo: MarchProgramInfo, buffers: BufferCollection, dt: number) {
...
function dispose(gl: WebGLRenderingContext, buffers: MarchBufferCollection, program: WebGLProgram): void {
  // delete interval
...
export async function run(gl: WebGLRenderingContext, canvasWidth: number, canvasHeight: number): Promise<UseGLContext> {
  ...
  const programInfo: MarchProgramInfo = {
  ...
  return {
    success: true,
    dispose: () => dispose(gl, buffers, programInfo.program),
